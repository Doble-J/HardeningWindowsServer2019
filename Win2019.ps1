...
# Microsoft Windows Server 2019 Benchmark

##########################################################################################################
$LogonLegalNoticeMessageTitle = ""
$LogonLegalNoticeMessage = ""

#Si existen requerimentos especiales de seguridad, se puede desabilitar las politicas:
$ExecutionList = @(
    #Mantener al inicio
    "CreateNewLocalAdminAccount", #Obligatorio, sino el acceso al sistema se pierde
    ...
    #2.2.36 No aplicable a servidor secundario
    #2.2.37 No aplicable a servidor secundario
    ...
    #2.2.47 No aplicable a servidor secundario
    ...
    #2.3.5.1 No aplicable a servidor secundario
    #2.3.5.2 No aplicable a servidor secundario
    #2.3.5.3 No aplicable a servidor secundario
    ...
    #2.3.10.6 No aplicable a servidor secundario
    ...
    #17.2.2 No aplicable a servidor secundario
    #17.2.3 No aplicable a servidor secundario
    #17.2.4 No aplicable a servidor secundario
    "AuditSecurityGroupManagement", #17.2.5
    "AuditUserAccountManagement", #17.2.6
    "AuditPNPActivity", #17.3.1
    "AuditProcessCreation", #17.3.2
    #17.4.1 No aplicable a servidor secundario
    #17.4.2 No aplicable a servidor secundario
    ...
    #18.2.1 to 12.2.6 No aplicable a servidor fuera del Dominio AD
	...
	#18.3.5 No aplicable a servidor secundario
...
#Crea al azar las nuevas cuentas de administrador e invitado en cada sistema.
#Esto aumenta la seguridad sin afectar la accesibilidad ya que estas cuentas siempre estan desactivadas. 
...
#No cambiar nada debajo de esta linea si no sabes lo que haces
##########################################################################################################
...
function CheckError([bool] $result, [string] $message) {
	# Verifica el valor del resultado y termina el
	# script despues de imprimir el mensaje de error especifico 
	# si el resultado especifico es falso.
    if ($result -eq $false) {
...
Write-Info "Script escrito por DobleJ. Basado en el script encontrado https://github.com/Cloudneeti/os-harderning-scripts/blob/master/WindowsServer2019/CIS_Benchmark_WindowsServer2019_v100.ps1"

function RegKeyExists([string] $path) {
	# Verifica si la clave de registro especificada existe
	$result = Get-Item $path -ErrorAction SilentlyContinue
	$?
}

function SetRegistry([string] $path, [string] $key, [string] $value, [string] $keytype) {
	# Establece el valor de registro especificado en la ruta especificada al valor especificado.
	# Primero, el valor original es leido e imprimido en la consola.
	# Si el valor original no existe, se verifica adicionalmente si la clave de
	# registro correspondiente existe, si no existe, la clave es creada, si no la llamada 
	# de Set-ItemProperty fallara.
	#
	# Ver: https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_commonparameters?view=powershell-7
	# Ver: https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_automatic_variables?view=powershell-7

    $before = Get-ItemProperty -Path $path -Name $key -ErrorAction SilentlyContinue
	
	if ($?) {
		Write-Before "Era: $($before.$key)"
	}
	else {
        Write-Before "Era: No Definido!"
		$keyExists = RegKeyExists $path
		
		if ($keyExists -eq $false) {
            Write-Info "Creando clave de registro '$($path)'."
			New-Item $path -Force -ErrorAction SilentlyContinue
            CheckError $? "Creando clave de registro '$($path)' fallo."
		}
	}

    Set-ItemProperty -Path $path -Name $key -Value $value -Type $keytype -ErrorAction SilentlyContinue

    CheckError $? "Creando valor del registro '$($path):$($value)' fallo."
    
    $after = Get-ItemProperty -Path $path -Name $key -ErrorAction SilentlyContinue
    Write-After "Ahora es: $($after.$key)"
}
...
function SetSecEdit([string]$role, [string[]] $values, $area, $enforceCreation) {
    ...
    if($null -eq $values) {
        Write-Error "SetSecEdit: Al menos un valor debe ser dado para establecer los roles:$($role)"
        return
    }
	...
	CheckError $? "Exportando '$($area)' to $(${env:appdata})\secpol.cfg' fallo."
    ...
    for($i =0; $i -lt $lines.Length; $i++) {
        if($lines[$i].Contains($role)) {
            Write-Before "Era: $($lines[$i])"
            $lines[$i] = $config
            $valueSet = $true
            Write-After "Ahora es: $($lines[$i])"
        }
    }
    ...
    if($enforceCreation -eq $true){
        if($valueSet -eq $false) {
            Write-Before "Era: No Definido"
            $lines += $config
            Write-After "Ahora es: $($lines[$lines.Length -1])"
        }
    }
	...
	CheckError $? "Configurando '$($area)' via $(${env:appdata})\secpol.cfg' fallo."
...
function CreateNewLocalAdminAccount {
    ...
    #Las funciones son practicamente un copia y pega del manual de Hardening para Windows Server 2019.
    #...y si, me dio flojera traducirlo.
...
{
    Write-Error "El script debe ser ejecutado en base x64"
    Write-Error "Abortado."
    return 1;
}
if(([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]"Administrator")) {
    ...
    if($NewLocalAdminUsername -ne "") {
        if($NewLocalAdminPswd -eq "") {
            Write-Error "NewLocalAdminUsername establecido, pero NewLocalAdminPasswd no."
            Write-Error "Favor de usar -NewLocalAdminPassword parametro para establecer el password!"
            return
        } else {
            if((ValidatePasswords $NewLocalAdminPswd $NewLocalAdminPswd) -eq $False) {
                Write-Error "NewLocalAdminPassword no cumple con los requerimientos minimos de seguridad"
                Write-Info "El password deben contener minimo 15 caracteres, mayusculas, numeros y simbolos"
                return 1;
            } else {
        ...
        do {
            Write-Info "Creare la nueva cuenta de Administrador, pero debes establecer el password para la nueva cuenta."
            Write-Info "El password deben contener minimo 15 caracteres, mayusculas, numeros y simbolos"
            Write-Info "Favor de escribir el nuevo password:"
            $temp_pass1 = Read-Host
            Write-Info "Repite el nuevo password:"
            $temp_pass2 = Read-Host 
            $invalid_pass = ValidatePasswords $temp_pass1 $temp_pass2 
            if($invalid_pass -eq $false) {
                Write-Error "El password no coincide o no cumple con los requisitos. Intenta otra vez."
            } else {
    ...
    if($LegalNoticeMessageFile -ne "") {
        if(Test-Path($LegalNoticeMessageFile) -eq $False) {
            Write-Error "El scrip no puede continuar, LegalNoticeMessageFile no fue encontrado"
            return 1;
        }
        ...
        if($legalNoticeFileContent.Length -ne 2) {
            Write-Error "El scrip no puede continuar, LegalNoticeMessageFile debe contener dos lineas, siendo la primera el Titulo y la segunda el mensaje"
            return 1;
        }
    ...
    if($ExecutionListFile -ne "") {
        if(Test-Path($ExecutionListFile) -eq $False) {
            Write-Error "Se proporciono un archivo de lista de ejecucion como parametro pero no se pudo encontrar! Abortando."
            return 1;
        }
